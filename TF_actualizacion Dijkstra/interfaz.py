...
def ventana2():
    ...
    radio_a = Radiobutton(contenedor, text="BFS", variable = opcion, value = "Algoritmo_A")
    radio_a.pack()

    radio_b = Radiobutton(contenedor, text="DIJKSTRA", variable = opcion, value = "Algoritmo_B")
    radio_b.pack()
...
def ventana_B(_origen, _destino):
    camino, precios = algoritmo.mi_grafo.Dijkstra(_origen, _destino)
    _camino = []
    ...
    imagen = PhotoImage(file = grafo.dibujar_Dijkstra(camino))
    imagen_redimensionada = imagen.subsample(3, 3)
    #imagen redimensionada
