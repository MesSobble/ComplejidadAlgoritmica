...
def dibujar_Dijkstra(camino):
    ...
    pos = nx.spring_layout(bfs_tree, seed=42)  # Posición de los nodos
    nx.draw(bfs_tree, pos, with_labels=True, node_size=200, font_size=24, font_color='black')
    plt.title(f'Dijkstra desde {camino[0][0]} hasta {camino[n-1][0]}')
    #plt.show()
    ...
    pos = nx.spring_layout(bfs_tree, seed=42)  # Posición de los nodos
    nx.draw(bfs_tree, pos, with_labels=True, node_size=200, font_size=24, font_color='black')
    plt.title(f'Subgrafo Dijkstra desde {camino[0][0]}')
    ...
    # Guardar la imagen en un archivo temporal (en el directorio actual)
    img_filename = f"pictures/grafos/Dijkstra_{camino[0][0]}_{camino[n-1][0]}_temp.png"
    plt.savefig(img_filename, format="png")
    plt.close()
...
#dibujar_grafo('Estación 1')
#dibujar_Dijkstra([('Estación 3', 0), ('Bus 1', 2.5), ('Estación 5', 1.5)])